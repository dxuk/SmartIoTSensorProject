...
using Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.Cabin.Devices.Factory;
using Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.Cabin.Telemetry.Factory;
using Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.Camera.Devices.Factory;
using Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.Camera.Telemetry.Factory;
...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator
{
    public static class Program
    {
        ...
        static void StartSimulator()
        {
            ...
            var cabinTelemetryFactory = new CabinTelemetryFactory(logger);
            var cameraTelemetryFactory = new CameraTelemetryFactory(logger);
            ...
            IDeviceFactory cabinDeviceFactory = new CabinDeviceFactory();
            IDeviceFactory cameraDeviceFactory = new CameraDeviceFactory();

            // Start Simulator
            Trace.TraceInformation("Starting Simulator");

            var cabinTester = new CabinBulkDeviceTester(transportFactory, logger, configProvider, cabinTelemetryFactory, cabinDeviceFactory, deviceStorage);
            var cameraTester = new CameraBulkDeviceTester(transportFactory, logger, configProvider, cameraTelemetryFactory, cameraDeviceFactory, deviceStorage);

            Task.Run(() => cabinTester.ProcessDevicesAsync(cancellationTokenSource.Token), cancellationTokenSource.Token);
            Task.Run(() => cameraTester.ProcessDevicesAsync(cancellationTokenSource.Token), cancellationTokenSource.Token);
        }
