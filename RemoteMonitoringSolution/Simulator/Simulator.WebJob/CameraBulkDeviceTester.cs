...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob
{
    ...
    /// </summary>
    public class CameraBulkDeviceTester
    {
        ...
        public CameraBulkDeviceTester(ITransportFactory transportFactory, ILogger logger, IConfigurationProvider configProvider,
        ...
        public async Task ProcessDevicesAsync(CancellationToken token)
        {
            ...
            try
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    {
                        newDevices = devices.Where(d => !_deviceList.Any(x => x.DeviceId == d.DeviceId) && d.DeviceId.Contains("Camera")).ToList();
                        removedDevices =
