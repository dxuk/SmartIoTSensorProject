...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.DeviceAdmin.Infrastructure.BusinessLogic
{
    public class DeviceLogic : IDeviceLogic
    {
        ...
        public async Task GenerateNCabinDevices(int deviceCount)
        {
            ...
            for (int i = 0; i < deviceCount; i++)
            {
                ...
                DeviceModel device = SampleDeviceFactory.GetSampleCabinDevice(randomNumber, generatedSecurityKeys);
        ...
        /// <returns></returns>
        /// <remarks>TEMPORARY DEVICE GENERATION CODE FOR TESTING PURPOSES!</remarks>
        public async Task GenerateNCameraDevices(int deviceCount)
        {
            Random randomNumber = new Random();
            ...
            {
                SecurityKeys generatedSecurityKeys = _securityKeyGenerator.CreateRandomKeys();
                DeviceModel device = SampleDeviceFactory.GetSampleCameraDevice(randomNumber, generatedSecurityKeys);
                await this.AddDeviceToRepositoriesAsync(device, generatedSecurityKeys);
            }
        }

        public async Task<List<string>> BootstrapDefaultCabinDevices()
        {
            List<string> sampleIds = SampleDeviceFactory.DefaultCabinNames;

            foreach(string id in sampleIds)
            {
                DeviceModel device = DeviceCreatorHelper.BuildCabinDeviceStructure(id, true, null);
                SecurityKeys generatedSecurityKeys = _securityKeyGenerator.CreateRandomKeys();
                await this.AddDeviceToRepositoriesAsync(device, generatedSecurityKeys);
            }
        ...
        public async Task<List<string>> BootstrapDefaultCameraDevices()
        {
            List<string> sampleIds = SampleDeviceFactory.DefaultCameraNames;
