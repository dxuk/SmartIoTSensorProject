...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.DeviceAdmin.Web.WebApiControllers
{
    ...
    public class DeviceApiController : WebApiControllerBase
    {
        ...
        // POST: api/v1/devices/samplecabin/5
        [HttpPost]
        [Route("samplecabin/{count}")]
        [WebApiRequirePermission(Permission.AddDevices)]
        public async Task<HttpResponseMessage> GenerateSampleCabinDevicesAsync(int count)
        {
            ValidatePositiveValue("count", count);

            return await GetServiceResponseAsync(async () =>
            {
                await _deviceLogic.GenerateNCabinDevices(count);
                return true;
            });
        }

        // POST: api/v1/devices/samplecamera/5
        [HttpPost]
        [Route("samplecamera/{count}")]
        [WebApiRequirePermission(Permission.AddDevices)]
        public async Task<HttpResponseMessage> GenerateSampleCameraDevicesAsync(int count)
        {
            ValidatePositiveValue("count", count);

            return await GetServiceResponseAsync(async () =>
            {
                await _deviceLogic.GenerateNCameraDevices(count);
                return true;
            });
        }
